...
class WorkerHandle(
  ...
  private inner class ServiceBindingImpl<T : WorkerService>(private val deferred: Deferred<WorkerMessage.BoundInstance>) : WorkerServiceBinding<T> {
    ...
    override suspend fun invoke(functionName: String, vararg args: JsonElement): JsonElement {
      ...
      val response = worker.sendMessage<WorkerMessage.FunctionCall, WorkerMessage.FunctionResult>(
  ...
  private suspend inline fun <reified T : WorkerMessage, reified R : WorkerMessage> Worker.sendMessage(message: T): R  = suspendCancellableCoroutine { continuation ->
