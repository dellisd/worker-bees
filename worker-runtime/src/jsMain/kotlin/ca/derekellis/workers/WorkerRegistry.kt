...
class WorkerRegistry {
  ...
  private fun newBoundInstance(bind: WorkerMessage.Bind): WorkerMessage.BoundInstance {
    val registration =
      factoryRegistry[bind.className]
        ?: throw IllegalStateException("No factory binding found for ${bind.className}")

    val newInstanceId = "${registration.typeClass.simpleName}-${idCounter++}"
    instanceRegistry[newInstanceId] =
      RegisteredInstance(registration.instanceClass.createInstance(), registration)
  ...
  private suspend fun handleFunctionCall(
    ...
    val registeredInstance =
      instanceRegistry[functionCall.instanceId]
        ?: throw IllegalStateException("No instance found for ${functionCall.instanceId}")

    val result =
      registeredInstance.registration.invocationHandler.handle(
  ...
  inline fun <reified T : WorkerService> register(
    clazz: KClass<out T>,
    invocationHandler: InvocationHandler<T>,
  ) {
    register(T::class, clazz, invocationHandler)
  }

  @Suppress("UNCHECKED_CAST")
  fun <T : WorkerService> register(
    typeClass: KClass<T>,
    instanceClass: KClass<out T>,
    invocationHandler: InvocationHandler<T>,
  ) {
    console.log("Registering ${typeClass.simpleName} for ${instanceClass.simpleName}")
    factoryRegistry[typeClass.simpleName!!] =
      Registration(typeClass, instanceClass, invocationHandler as InvocationHandler<WorkerService>)
  }

  private data class Registration(
    val typeClass: KClass<out WorkerService>,
    val instanceClass: KClass<out WorkerService>,
