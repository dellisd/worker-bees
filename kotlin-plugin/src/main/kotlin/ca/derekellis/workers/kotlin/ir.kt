// https://github.com/cashapp/zipline/blob/0e122c08e800772430d167d987d07bfbb86d9a7d/zipline-kotlin-plugin/src/main/kotlin/app/cash/zipline/kotlin/ir.kt
  ...
  get() = buildString {
    ...
    if (property != null) {
      ...
      if (parameters.size == 2) {
        // setter
        append("var $name: ${(parameters[1].type as IrSimpleType)}")
      } else {
        // getter
        append("val $name: ${(returnType as IrSimpleType)}")
      }
    } else {
      if (isSuspend) {
        append("suspend ")
  ...
  result.getter =
    irFactory
      .createSimpleFunction(
        ...
        visibility =
          overriddenProperty?.owner?.getter?.visibility ?: DescriptorVisibilities.PRIVATE,
        isInline = false,
        isExpect = false,
      ...
      .apply {
        ...
        parameters += buildReceiverParameter { type = declaringClass.defaultType }
        irFunctionBody(context = pluginContext, scopeOwnerSymbol = symbol) {
          +irReturn(value = irGetField(irGet(dispatchReceiverParameter!!), result.backingField!!))
        }
      }
...
internal fun IrBuilderWithScope.irKClass(containerClass: IrClass): IrClassReferenceImpl {
  ...
  companionClass
    ...
    .apply {
      irConstructorBody(irPluginContext) { statements ->
        ...
        statements +=
          irInstanceInitializerCall(context = irPluginContext, classSymbol = companionClass.symbol)
      }
    }
